...
const UpdateExperience = () => {
  const { updateExperience, alertMessage, getExperience } = useExperience();
  ...
  const fetchProject = useCallback(async () => {
    if (params.id && !hasFetched) {
      try {
        const education = await getExperience(params.id);
  ...
  }, [getExperience, params.id, hasFetched, setValue]);
  ...
  const onSubmit = async (data) => {
    try {
      ...
      await updateExperience(params.id, formattedData);
